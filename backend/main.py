...
Registers module routers and configures middleware, exception handlers,
and the application lifespan hooks.
"""
...
async def lifespan(app: FastAPI):
    ...
    # Best-effort pre-warm of embedding model to avoid first-request latency.
    try:
        ...
        logger.debug("Could not pre-warm embedding model: %s", exc)
    yield
...
# ---------------------------------------------------------------------------
# Routers (optional imports guarded to allow running without all modules)
# ---------------------------------------------------------------------------
...
try:
    ...
    logger.debug("AI routes not available; skipping")
...
try:
    ...
    logger.debug("Auth routes not available; skipping")
...
try:
    ...
    logger.debug("Database routes not available; skipping")
...
try:
    ...
    logger.debug("Security routes not available; skipping")
